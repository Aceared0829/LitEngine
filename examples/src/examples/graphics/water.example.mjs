...
const assets = {
    island: new Asset('island', 'container', { url: './assets/models/tropical-island.glb' }),
    normal: new Asset('normal', 'texture', { url: './assets/textures/normal-map.png' }),
    caustics: new Asset('caustics', 'texture', { url: './assets/textures/caustics.jpg' })
};
...
const waterLayer = new Layer({ name: 'Water' });
...
camera.addComponent('camera', {
    fov: 60,
    farClip: 3000,
    layers: [worldLayer.id, LAYERID_DEPTH, skyboxLayer.id, waterLayer.id, uiLayer.id]
});
...
cameraControls.focusPoint = new Vec3(8, 0, -5);
// allow diving below the water surface
cameraControls.pitchRange = new Vec2(-90, 89);
cameraControls.zoomRange = new Vec2(2, 150);
...
const elevationCurve = new Curve([0, -60, 6, 0, 12, 60, 18, 0, 24, -90]);
elevationCurve.type = CURVE_SMOOTHSTEP;

// sky luminance by sun elevation - brighter sky radiance at twilight, dimmer at midday
const luminanceCurve = new Curve([0, 2, 35, 0.4, 90, 0.3]);
luminanceCurve.type = CURVE_SMOOTHSTEP;

// bloom intensity by sun elevation - stronger glow around the low sun
const bloomCurve = new Curve([0, 0.005, 5, 0.001, 8, 0.001, 90, 0.002]);
bloomCurve.type = CURVE_SMOOTHSTEP;
let lastBloom = -1;
...
const applyFogColor = () => {
    ...
    if (c) {
        app.scene.fog.color = new Color(c[0] * fogDayFactor, c[1] * fogDayFactor, c[2] * fogDayFactor);
    }
};
...
// water color presets
const presets = {
    tropical: { shallowColor: new Color(0.6, 0.9, 0.85), deepColor: new Color(0.03, 0.16, 0.24), depthFade: 3 },
    ocean: { shallowColor: new Color(0.35, 0.55, 0.6), deepColor: new Color(0.01, 0.06, 0.12), depthFade: 6 },
    murky: { shallowColor: new Color(0.5, 0.55, 0.35), deepColor: new Color(0.1, 0.14, 0.05), depthFade: 1.2 }
};
