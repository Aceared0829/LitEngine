...
/**
 * @import { LayerComposition, Material, RenderComponent, ShaderMaterial } from 'playcanvas'
 */
...
const assets = {
    ...
    inside: new Asset('inside', 'gsplat', { url: INSIDE_URL }),
...
app.scene.gsplat.splatBudget = platform.mobile ? 500000 : 1500000;
...
const portalMaskLayer = new Layer({ name: 'PortalMask' });
const portalArchLayer = new Layer({ name: 'PortalArch' });
const outsideLayer = new Layer({ name: 'PortalOutside' });
const insideLayer = new Layer({ name: 'PortalInside' });
...
const portalGlassLayer = new Layer({ name: 'PortalGlass' });

// ---- Layer ordering with explicit opaque/transparent control ----
// LayerComposition.insert() inserts BOTH the opaque and transparent
...
camera.addComponent('camera', {
    clearColor: new Color(0.05, 0.05, 0.07),
    layers: [
        outsideLayer.id,
        portalMaskLayer.id,
        portalArchLayer.id,
...
const focusPoint = new Vec3(12, 2, 0);
...
const PORTAL_WORLD_POS = new Vec3(12, 1.5, 0);
const PORTAL_WORLD_YAW = 180;

const portalGroup = new Entity('Portal');
...
const sceneConfigs = [
    {
        ...
        portalLocalYaw: 130,
        viewThroughOffset: new Vec3(0, 0, 0)
    },
    {
        name: 'Skatepark',
        asset: assets.inside,
...
const trsYaw = (/** @type {Vec3} */ pos, /** @type {number} */ yaw) => {
    return new Mat4().setTRS(pos, new Quat().setFromEulerAngles(0, yaw, 0), Vec3.ONE);
};
...
const applyMatToEntity = (/** @type {Entity} */ entity, /** @type {Mat4} */ mat) => {
...
const buildOriginalTransform = (/** @type {typeof sceneConfigs[number]} */ config) => {
    ...
    const intrinsicMat = new Mat4().setTRS(
        Vec3.ZERO,
 ...
 * @property {Entity} entity - The splat entity for this scene.
 * @property {Mat4} fromTransform - world transform used while the user is *in* this scene (content wraps around the camera).
 * @property {Mat4} throughTransform - world transform used while this scene is viewed *through* the portal.
 */
...
sceneConfigs.forEach((config) => {
    ...
    const throughOffsetMat = new Mat4().setTRS(config.viewThroughOffset, new Quat(), Vec3.ONE);
    const throughTransform = new Mat4().mul2(throughOffsetMat, fromTransform);
...
const maskStencil = new StencilParameters({
    zpass: STENCILOP_INCREMENT
});
const archStencil = new StencilParameters({
    func: FUNC_ALWAYS,
    ref: 0,
    zpass: STENCILOP_ZERO
});
const portalOnlyStencil = new StencilParameters({
    func: FUNC_NOTEQUAL,
    ref: 0
});
...
const maskMaterial = new StandardMaterial();
...
maskMaterial.cull = CULLFACE_NONE;
maskMaterial.stencilFront = maskMaterial.stencilBack = maskStencil;
maskMaterial.update();

const maskPlane = new Entity('PortalMaskPlane');
...
portalEntity.findComponents('render').forEach((/** @type {RenderComponent} */ render) => {
    render.layers = [portalArchLayer.id];
    for (const meshInstance of render.meshInstances) {
        archMaterials.add(meshInstance.material);
    }
});
archMaterials.forEach((/** @type {Material} */ mat) => {
...
glassMaterial.diffuse = new Color(0, 1, 1);
glassMaterial.specular = new Color(0.4, 0.4, 0.4);
...
const applyMaterialStateForLayer = (/** @type {ShaderMaterial} */ material, /** @type {Layer} */ layer) => {
    const through = isLayerThrough(layer);
    material.stencilFront = material.stencilBack = through ? portalOnlyStencil : null;
    material.depthFunc = through ? FUNC_GREATER : FUNC_LESSEQUAL;
    material.update();
};
...
const prevCamWorldPos = new Vec3().copy(camera.getPosition());
const localPrev = new Vec3();
const localCurr = new Vec3();
const planeWorldInv = new Mat4();
